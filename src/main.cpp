...
int nStakeMinConfirmations = 1400;              // 1400 Confirmations before mature
...
int64_t GetProofOfStakeReward(const CBlockIndex* pindexPrev, int64_t nCoinAge, int64_t nFees, int64_t SupermoonCoinStake )
{
	...
	if(pindexBest->GetBlockTime()  >= 1530863459)
		...
		if(pindexBest->GetBlockTime()  <= 1530863460)                                
	        {
	        nSubsidy = SUPMCoinWeightMax * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 1;    
                LogPrintf("Week of Jul-6-13 nSubsidy=%d, Supermoontime=%d \n\n", nSubsidy, pindexBest->GetBlockTime() );
		}
	
		else if(pindexBest->GetBlockTime()  <= 1531455223)  
	        {
	        nSubsidy = SUPMCoinWeightMax * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 7; 
			LogPrintf("--------------------------------------------------------------------------------------\n");
			LogPrintf("July 13-18 ECLIPSE  01:48:17 to 04:13:43 UTC nSubsidy=%d, Supermoontime=%d \n", nSubsidy, pindexBest->GetBlockTime());
			LogPrintf("--------------------------------------------------------------------------------------\n");
		}
                else if(pindexBest->GetBlockTime()  <= 1531541580)  
	        {
	        nSubsidy = SUPMCoinWeightMax * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 8; 
			LogPrintf("--------------------------------------------------------------------------------------\n");
			LogPrintf("July 13-18 SUPERMOON NEW -  ECLIPSE & ENDS  04:13:43 UTC nSubsidy=%d, Supermoontime=%d \n", nSubsidy, pindexBest->GetBlockTime());
			LogPrintf("--------------------------------------------------------------------------------------\n");
		}
		else if(pindexBest->GetBlockTime()  <= 1532029920) 
	        {
	        nSubsidy = SUPMCoinWeightMax * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 1; 
			LogPrintf("--------------------------------------------------------------------------------------\n");
	        ...
	        {
	        nSubsidy = SUPMCoinWeightMax * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 2; 
            LogPrintf("Week of July 19-27 1/2 nSubsidy=%d, Supermoontime=%d \n\n", nSubsidy, pindexBest->GetBlockTime() );
		}
		else if(pindexBest->GetBlockTime()  <= 1532734118)  
	        {
	        nSubsidy = SUPMCoinWeightMax * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 7; 
			LogPrintf("--------------------------------------------------------------------------------------\n");
		...
		else if(pindexBest->GetBlockTime()  <= 1533406680)   
	        {
		...
		else if(pindexBest->GetBlockTime()  <= 1533987000)  
	        {
		...
		else if(pindexBest->GetBlockTime()  <= 1534067880)  
	        {
	        nSubsidy = SUPMCoinWeightMax * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 8;         
			LogPrintf("--------------------------------------------------------------------------------------\n");
		...
		else if(pindexBest->GetBlockTime()  <= 1534578480)  
	        {
            		nSubsidy = SUPMCoinWeightMax * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 1;      
			LogPrintf("--------------------------------------------------------------------------------------\n");
	...
	if(pindexBest->GetBlockTime()  >= 1535942221)
    	{
           ...
           if(pindexBest->GetBlockTime()  <= 1536516060)
	        ...
	        {
			nSubsidy = SUPMCoinWeightMax * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 4;        
	            	LogPrintf("--------------------------------------------------------------------------------------\n");
        	...
        	LogPrintf("Week of Sep 25-Oct 2 - 2018 Full Moon  nSubsidy=%d, Supermoontime=%d \n\n", nSubsidy, pindexBest->GetBlockTime() );
       		...
       		else if(pindexBest->GetBlockTime()  <= 1538956800)			
		{
            	nSubsidy = SUPMCoinWeightMax * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 2;          
		...
		else if(pindexBest->GetBlockTime()  <= 1540080000)				
	        {
	        nSubsidy = SUPMCoinWeightMax * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 2;      
        	...
        	else if(pindexBest->GetBlockTime()  <= 1542293640)    	
		{
			...
			LogPrintf("--------------------------------------------------------------------------------------\n");           
	         ...
	         {
			nSubsidy = SUPMCoinWeightMax * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 10;         
	            	LogPrintf("--------------------------------------------------------------------------------------\n");
		...
		else if(pindexBest->GetBlockTime()  <= 1546746526)  
	        {
	        nSubsidy = SUPMCoinWeightMax * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 5;         
			LogPrintf("--------------------------------------------------------------------------------------\n");
	...
	if(pindexBest->GetBlockTime()  >= 1546746527)
    	{
		...
		else if(pindexBest->GetBlockTime()  <= 1548056882)  
	        {
	        nSubsidy = SUPMCoinWeightMax * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 13;         
			LogPrintf("--------------------------------------------------------------------------------------\n");
			LogPrintf("SUPERMOON Start & Lunar ECLIPSE 21/1/19 07:48 UTC & DUAL Stack Bonus nSubsidy=%d, Supermoontime=%d  \n", nSubsidy, pindexBest->GetBlockTime());
			LogPrintf("--------------------------------------------------------------------------------------\n");
		}				
	
		else if(pindexBest->GetBlockTime()  <= 1548134160)   
	        {
	        nSubsidy = SUPMCoinWeightMax * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 9;         
			LogPrintf("--------------------------------------------------------------------------------------\n");
		...
		else if(pindexBest->GetBlockTime()  <= 1550677980)  
	        {
	        nSubsidy = SUPMCoinWeightMax * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 9;         
			LogPrintf("--------------------------------------------------------------------------------------\n");
	        ...
	        nSubsidy = SUPMCoinWeightMax * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 1;     
	         ...
	         {
			nSubsidy = SUPMCoinWeightMax * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 3;         
	            	LogPrintf("--------------------------------------------------------------------------------------\n");
	...
	if(pindexBest->GetBlockTime()  >= 1562086513)
	   ...
	   if(pindexBest->GetBlockTime()  <= 1562094960)  
		...
		else if(pindexBest->GetBlockTime()  <= 1562104234)  
	        {
	        nSubsidy = SUPMCoinWeightMax * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 3;          
			LogPrintf("--------------------------------------------------------------------------------------\n");
		...
		else if(pindexBest->GetBlockTime()  <= 1563322658)  
	        {
	        nSubsidy = SUPMCoinWeightMax * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 8;         
			LogPrintf("--------------------------------------------------------------------------------------\n");
		...
		else if(pindexBest->GetBlockTime()  <= 1569781560)  
	        {
	        nSubsidy = SUPMCoinWeightMax * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 5;         
			LogPrintf("--------------------------------------------------------------------------------------\n");
		...
		else if(pindexBest->GetBlockTime()  <= 1577347540)  
	        {
	        nSubsidy = SUPMCoinWeightMax * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 4;         
			LogPrintf("--------------------------------------------------------------------------------------\n");
       	...
       	// SUPERMOON 2020 - 1st Jan 2020 11:11:11AM - 100% APR as standard... Will ajust add Moon Phases / Stellar events @ later release - SUPM Devs Oct 2018
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
	...
	//LogPrintf("nValueIn=%d,  nValueOut=%d  pindex->nMint=%d pindex->nMoneySupply=%d \n", nValueIn, nValueOut, pindex->nMint, pindex->nMoneySupply );   ///  SUPM-DEV
