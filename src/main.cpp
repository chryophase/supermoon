...
// Copyright (c) 2018 Supermoon Developers v1.0753 Prod  1.1.0.0
...
#if defined(NDEBUG)
# error "supermoon cannot be compiled without assertions."
#endif
...
int nStakeMinConfirmations = 40;                // 40 Confirmations
...
const string strMessageMagic = "supermoon Signed Message:\n";
...
////////////////////////////////////////////////////////////
// Supermoon Proof of Stake - Block Time functions
//
//  LogPrintf("- SUPM MoneySupply=%d \n",  FormatMoney(pindexBest->nMoneySupply));
...
int64_t GetProofOfStakeReward(const CBlockIndex* pindexPrev, int64_t nCoinAge, int64_t nFees)
{
    ...
    LogPrintf("****** Proof Of Stake Engine *******  \n");
    ...
    if (pindexBest->nHeight <= 3500) 
    	...
    	LogPrintf("**** \n");
	LogPrintf("* Less then 3500 - nSubsidy=%d nHeight=%d \n", nSubsidy, pindexBest->nHeight );
    	LogPrintf("**   \n");
       	}
    ...
    LogPrintf("** Supermoon! PoW Active - nCoinAge=%d nHeight=%d  \n", nCoinAge, pindexBest->nHeight );
    LogPrintf("**** \n");
	...
	///// Eclipse triggers
		else if(pindexBest->GetBlockTime()  <= 1531455223)  // 1531446498 July 13 Lunar ECLIPSE START 01:48:18
	        {
			...
			LogPrintf("--------------------------------------------------------------------------------------\n");
			LogPrintf("July 13-18 ECLIPSE  01:48:17 to 04:13:43 UTC nSubsidy=%d, Supermoontime=%d \n", nSubsidy, pindexBest->GetBlockTime());
			LogPrintf("--------------------------------------------------------------------------------------\n");
		}
		...
		else if(pindexBest->GetBlockTime()  <= 1532029920)  //July 14 SUPERMOON NEW 02:48 UTC - Starts New Moon
	        {
	        nSubsidy = SUPMCoinWeightMax * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 1; //July 13 New End of Eclipse - Starts New Moon
			LogPrintf("--------------------------------------------------------------------------------------\n");
  	        LogPrintf("July 14-18 New Moon -SUPERMOON NEW Ended 09:58- nSubsidy=%d, Supermoontime=%d \n", nSubsidy, pindexBest->GetBlockTime());
			LogPrintf("--------------------------------------------------------------------------------------\n");
		}
		else if(pindexBest->GetBlockTime()  <= 1532711687)
	        {
	        nSubsidy = SUPMCoinWeightMax * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 2; //July 19 First 1/2
            LogPrintf("Week of July 19-27 1/2 nSubsidy=%d, Supermoontime=%d \n\n", nSubsidy, pindexBest->GetBlockTime() );
		}
		else if(pindexBest->GetBlockTime()  <= 1532734118)  //July 27 Lunar ECLIPSE START 17:14:47
	        {
	        nSubsidy = SUPMCoinWeightMax * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 7; //July 27 Lunar ECLIPSE 17:14:47 to 23:28:38 UTC
			LogPrintf("--------------------------------------------------------------------------------------\n");
			LogPrintf("July 27-2018 Lunar ECLIPSE * FULL MOON 17:14:47 to 23:28:38 UTC nSubsidy=%d, Supermoontime=%d \n", nSubsidy, pindexBest->GetBlockTime());
			LogPrintf("--------------------------------------------------------------------------------------\n");
		}
		else if(pindexBest->GetBlockTime()  <= 1533406680)   //July 27 Full Moon has started during Eclipse - 1532722800
	        {
	        nSubsidy = SUPMCoinWeightMax * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 5;         //July 27 Full
            LogPrintf("Week of Jul 27-Aug 4 Full Moon nSubsidy=%d, Supermoontime=%d \n\n", nSubsidy, pindexBest->GetBlockTime());
		}
    /////
    //Aug 2018
	    	else if(pindexBest->GetBlockTime()  <= 1533974528)
            	{
	        nSubsidy = SUPMCoinWeightMax * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 2;         //Aug 4 Last 1/2
            	LogPrintf("Week of Aug 4-11 1/2 nSubsidy=%d, Supermoontime=%d \n\n", nSubsidy, pindexBest->GetBlockTime() );
		}
		else if(pindexBest->GetBlockTime()  <= 1533987000)  //Aug 11 Lunar ECLIPSE START 08:02:08
	        {
	        nSubsidy = SUPMCoinWeightMax * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 9;         //Aug 11 Lunar ECLIPSE 08:02:08 to 11:30:40 UTC
			LogPrintf("--------------------------------------------------------------------------------------\n");
			LogPrintf("Aug 11-2018 Lunar ECLIPSE 08:02:08 to 11:30:40 UTC & NEW SUPERMOON 09:58 UTC nSubsidy=%d, Supermoontime=%d  \n", nSubsidy, pindexBest->GetBlockTime());
			LogPrintf("--------------------------------------------------------------------------------------\n");
		}
		else if(pindexBest->GetBlockTime()  <= 1534067880)  //Aug 11 New SUPERMOON
	        {
	        nSubsidy = SUPMCoinWeightMax * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 8;         //Aug 11 Lunar ECLIPSE 08:02:08 to 11:30:40 UTC
			LogPrintf("--------------------------------------------------------------------------------------\n");
			LogPrintf("Aug 11-2018 SUPERMOON NEW 09:58: UTC nSubsidy=%d, Supermoontime=%d  \n", nSubsidy, pindexBest->GetBlockTime());
			LogPrintf("--------------------------------------------------------------------------------------\n");
		}
		else if(pindexBest->GetBlockTime()  <= 1534578480)  //Aug 12 END New SUPERMOON END
	        {
            		nSubsidy = SUPMCoinWeightMax * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 1;      //Aug 12 Standard New Moon Subsidy
			LogPrintf("--------------------------------------------------------------------------------------\n");
			LogPrintf("Week of Aug 12-18 NEW MOON - SUPERMOON NEW ENDED 12 AUG-09:58 UTC -  nSubsidy=%d, Supermoontime=%d \n\n", nSubsidy, pindexBest->GetBlockTime());
			LogPrintf("--------------------------------------------------------------------------------------\n");
		}
   ...
   // Oct 2018
      		else if(pindexBest->GetBlockTime()  <= 1538956800)			//Instead of 1539056820, Ajusted to start of DRACONIDS Bonus
		{
            	...
            	LogPrintf("Week of Oct 2-9 1/2 nSubsidy=%d, Supermoontime=%d \n\n", nSubsidy, pindexBest->GetBlockTime() );
		}
	//DRACONIDS
		else if(pindexBest->GetBlockTime()  <= 1539129540)
	         {
			nSubsidy = SUPMCoinWeightMax * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 4;         //Oct 8-9 (48 Hours)
	            	LogPrintf("--------------------------------------------------------------------------------------\n");
			LogPrintf("8-9th OCT-2018 DRACONIDS  00:01 UTC (48 HOURS) Bonus STACK APR +200 -  nSubsidy=%d, Supermoontime=%d \n\n", nSubsidy, pindexBest->GetBlockTime());
			LogPrintf("--------------------------------------------------------------------------------------\n");           
		 }
		else if(pindexBest->GetBlockTime()  <= 1539712920)
	        {
	        nSubsidy = SUPMCoinWeightMax * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 1;     //Oct 9 New
            	LogPrintf("Week of Oct 9-16 New nSubsidy=%d, Supermoontime=%d \n\n", nSubsidy, pindexBest->GetBlockTime() );
		}
	//ORIONIDS
		else if(pindexBest->GetBlockTime()  <= 1540080000)				// Oct 16 First 1/2 until ORIONIDS  Starts 21st OCT (48 Hours)
	        {
	        nSubsidy = SUPMCoinWeightMax * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 2;      // Oct 16 until 21st First 1/2  (ORIONIDS)
            	LogPrintf("Week of Oct 16-21 1/2 nSubsidy=%d, Supermoontime=%d \n\n", nSubsidy, pindexBest->GetBlockTime() );
		}
        	else if(pindexBest->GetBlockTime()  <= 1540252740)
	         {
			nSubsidy = SUPMCoinWeightMax * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 4;       //Oct 21-22 (48 Hours)
	            	LogPrintf("--------------------------------------------------------------------------------------\n");
			LogPrintf("21-22 OCT-2018 ORIONIDS 00:01 UTC (48 HOURS) Bonus STACK APR +200 -  nSubsidy=%d, Supermoontime=%d \n\n", nSubsidy, pindexBest->GetBlockTime());
			LogPrintf("--------------------------------------------------------------------------------------\n");           
		 }
	...
	//TAURIDS
	        ...
	        nSubsidy = SUPMCoinWeightMax * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 2;     //Nov 6-9 Last 1/2 of Moon before New Moon
            	LogPrintf("Week of Nov6- 9 1/2 nSubsidy=%d, Supermoontime=%d \n\n", nSubsidy, pindexBest->GetBlockTime() );
		}
    /////////////  PROD
    //Nov 2018
        	else if(pindexBest->GetBlockTime()  <= 1542387780)
		{
	        ...
	        LogPrintf("Week of Nov 9-16 New nSubsidy=%d, Supermoontime=%d \n\n", nSubsidy, pindexBest->GetBlockTime() );
		}
	//LEONIDS
	        ...
	        nSubsidy = SUPMCoinWeightMax * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 2;     //Nov 16-17 First 1/2 (Start of LEONIDS)
            	LogPrintf("Week of Nov 16-17 1/2 nSubsidy=%d, Supermoontime=%d \n\n", nSubsidy, pindexBest->GetBlockTime() );
		}
		else if(pindexBest->GetBlockTime()  <= 1542585540)
	         {
			nSubsidy = SUPMCoinWeightMax * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 4;   //NOV 17-18 (48 Hours)
	            	LogPrintf("--------------------------------------------------------------------------------------\n");
    ...
    //Dec 2018
	        ...
	        nSubsidy = SUPMCoinWeightMax * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 1;     //Dec 7-13  New (Geminids Start
            	LogPrintf("Week of Dec 7-13 New nSubsidy=%d, Supermoontime=%d \n\n", nSubsidy, pindexBest->GetBlockTime() );
		}
	//GEMINIDS
		else if(pindexBest->GetBlockTime()  <= 1544831940)
	         {
			nSubsidy = SUPMCoinWeightMax * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 4;   //DEC 13-14 (48 Hours)
	            	LogPrintf("--------------------------------------------------------------------------------------\n");
			LogPrintf("13-14 Dec-2018 GEMINIDS 00:01 UTC (48 HOURS) +Bonus STACK APR +200 -  nSubsidy=%d, Supermoontime=%d \n\n", nSubsidy, pindexBest->GetBlockTime());
			LogPrintf("--------------------------------------------------------------------------------------\n");           
		 }
		else if(pindexBest->GetBlockTime()  <= 1544874540)
            	{
	        nSubsidy = SUPMCoinWeightMax * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 1;     //Dec 7 New
            	LogPrintf("Week of Dec 14-15 New Moon nSubsidy=%d, Supermoontime=%d \n\n", nSubsidy, pindexBest->GetBlockTime() );
		}

		else if(pindexBest->GetBlockTime()  <= 1545350400)
	        {
	        nSubsidy = SUPMCoinWeightMax * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 2;     //Dec 15-21 First 1/2
            	LogPrintf("Week of Dec 15-21 1/2 Moon nSubsidy=%d, Supermoontime=%d \n\n", nSubsidy, pindexBest->GetBlockTime() );
		}
         //URSIDS 
	...
	//URSIDS & SOLSTICE (2x Stack Bonus) still 1/2 Moon!
			...
			nSubsidy = SUPMCoinWeightMax * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 7;         //Dec 22  URSIDS & SOLSTICE start  (18 Hours Period)
	            	LogPrintf("--------------------------------------------------------------------------------------\n");
			LogPrintf("SOLSTICE 21-DEC 22:22 (24 HOURS) & URSIDS with 1/2 Moon +Bonus STACK APR +400 + DOUBLE +100-  nSubsidy=%d, Supermoontime=%d \n\n", nSubsidy, pindexBest->GetBlockTime());
			LogPrintf("--------------------------------------------------------------------------------------\n");           
		 }	
	...
	//URSIDS & Full Moon 	
	        ...
	        nSubsidy = SUPMCoinWeightMax * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 5;     //Dec 22 Full
            	LogPrintf("Week of Dec 22-29 Full Moon nSubsidy=%d, Supermoontime=%d \n\n", nSubsidy, pindexBest->GetBlockTime() );
		}
	        ...
	        nSubsidy = SUPMCoinWeightMax * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 2;     //Dec 29 2018 to 3rd Jan 2019  Last 1/2 Moon
            	LogPrintf("Week of Dec 29-Jan 3 1/2 Moon nSubsidy=%d, Supermoontime=%d \n\n", nSubsidy, pindexBest->GetBlockTime() );
		}
	//Quadrantids
	        ...
	        nSubsidy = SUPMCoinWeightMax * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 2;     //Jan 4-6 2019  Last 1/2 Moon
            	LogPrintf("Week of 4-6 Jan 1/2 Moon nSubsidy=%d, Supermoontime=%d \n\n", nSubsidy, pindexBest->GetBlockTime() );
		}
	    else     	// Report if no expressions matched
	        {
	        LogPrintf("** End SUPM Dec 2018 Nest - No expressions Matc \n");
		}
    ////// End of 2018
    LogPrintf("** End SUPM 2018 Nest \n");
   ...
   //////////////////////////////
	...
	//Jun 2019
	        ...
	        {
	        LogPrintf("** End SUPM Jun 2019 Nest - No expressions Match \n");
		}
	...
	//Dec 2019
	        ...
	        {
	        LogPrintf("** End SUPM Dec 2019 Nest - No expressions Match \n");
		}
    	////// End of 2019
	LogPrintf("*******************************\n");	
   	LogPrintf("*** End SUPM 2019 Nest Reached \n");
	LogPrintf("*******************************\n\n");	
          
          
       ////////////////////////
       // SUPERMOON 2020 Phase Check 1st Jan 2020 11:11:11AM ------> Effort reward set to 100% as standard onwards... (SUPM Devs) 
       ////////////////////////
       if(pindexBest->GetBlockTime()  >= 1577877071    )
	        {
        ...
        //////////////////////////////////////////////////////////
        /////   End of SUPERMOON EPOCH Nest 2018 to March 2019  //
        //////////////////////////////////////////////////////////
	...
	LogPrintf("** End Calcs - Final result nSubsidy=%d nHeight=%d \n", nSubsidy, pindexBest->nHeight );
 	...
 	if(pindexBest->nMoneySupply >= 49999999000000000)         // Teest for > 499.999 Million MAX Supply Target (approx 500,000,000 SUPM - Last 8 '0's are Decimal places)
 		{
         		nSubsidy = 1; 	//Once last base 10 SUPM coins passeed, PoS reward is flat (1 SAT - Fee) Coin mechanic still allows PoS engines to stake, just no APR calc.
       		  	LogPrintf("*****************************************************************\n");
...
bool CTransaction::GetCoinAge(CTxDB& txdb, const CBlockIndex* pindexPrev, uint64_t& nCoinAge) const
{
    ...
    LogPrintf("************** nCoinAge **********************************\n");
    LogPrintf("Calculated Coinage is nCoinAge coin age = bnCoinDay=%s\n", bnCoinDay.ToString());
    LogPrintf("************** End nCoinAge Calc**********************************\n");
...
{
    RenameThread("supermoon-loadblk");
