...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("Supermoon"))
        return false;
...
{
    // Convert Supermoon:// to Supermoon:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Supermoon://"))
    {
        uri.replace(0, 12, "Supermoon:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Supermoon.lnk";
}
...
{
    return GetAutostartDir() / "Supermoon.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Supermoon-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Supermoon-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("Supermoon-Qt"));
...
// background-image: url(:/images/supermoonbak);
